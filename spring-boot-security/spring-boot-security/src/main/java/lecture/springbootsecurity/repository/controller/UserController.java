...
@RestController
@RequestMapping("/auth")
@Slf4j // 로그를 편리하게 찍게 도와주는 annotation
    ...
    public ResponseEntity<?> registerUser(@RequestBody UserDTO userDTO) {
        try {
            UserEntity user = UserEntity.builder()
                    ...
                    .password(passwordEncoder.encode(userDTO.getPassword())) // 비밀번호를 암호화 해서 넘겨줄거임.
                    .build();
...
//            log.warn()
            ...
            session.setAttribute("userId", user.getId()); // session 에 PK 인 userId를 set 한다.
            return ResponseEntity.ok().body(responseUserDTO);
        } catch (Exception e){
            return ResponseEntity.badRequest().body(e.getMessage());
        }
